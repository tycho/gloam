...
files (by repository, commit, and content hash). This guide is
...
- **Generated from the following upstream sources** — grouped by repository.
  The repository line carries the short commit; each file under it carries its
...
(same `path_in_repo`s and `blob`s) keeps its previous `commit`
...
- **Content is always current.** With `--fetch`, file content still comes from
  upstream HEAD; only the *recorded* commit is carried forward, and
