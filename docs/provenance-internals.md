...
1. Record the exact upstream provenance of every input that influences a
   generated loader: repository, commit, and content (blob)
   hash.
...
-- Commit rows exist for eviction bookkeeping (orphaned tree entries are
-- swept when their commit is evicted).
commits(commit_sha TEXT PRIMARY KEY, repo TEXT, last_used INTEGER)
...
- **`--fetch --lock`:** never touches `repos` (HEAD). Keys off the manifest's
  ...
  commit/tree rows. Bumps `last_used` on
  everything reused.
...
(repo, commit, blob, path), so a run where every required file hits the
...
Keep fetched provenance purely factual (repo, commit, blob). Copyright
...
footer. The two-level source layout (repo+commit on one line, indented
