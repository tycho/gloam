...
### Manifest-only snapshot subcommand

A gloam subcommand that resolves **all clusters / all known files** and writes a
provenance-only manifest (no loader output). This is the reusable point-in-time
lock referenced in the consumer guide; it shares the acquisition path with the
xtask and `--fetch`.
