...
impl TypeRef {
    ...
    /// "no value".  Also matches windows.h's `VOID` macro spelling, which
    /// wgl.xml uses for a few command returns — `VOID` is only ever plain
    /// (pointer-typed uses go through `LPVOID`).
    pub fn is_void(&self) -> bool {
        (self.base == "void" || self.base == "VOID")
