...
pub fn generate(
    ...
    for fs in feature_sets {
        if !matches!(fs.spec, Spec::Gl | Spec::Vk | Spec::Egl) {
            bail!(
                "the rust backend currently supports the GL/GLES, Vulkan, and \
                 EGL APIs; {} is not implemented yet",
                fs.display_name
            );
        }
    }
    if args.mx_global && !feature_sets.iter().any(|fs| fs.spec == Spec::Gl) {
        bail!("--mx-global is only implemented for the GL rust backend so far");
    }
