//! Vulkan type emission for the Rust backend.
//!
...
//!   vk_video headers, outside vk.xml; the referenced names are declared
//!   from their usage shapes (enums as `i32`, records opaque — see
//!   `collect_std_video`), with the enum assumption pinned by the layout
//!   oracle against the real headers.
...
fn platform_type_decl(name: &str) -> Option<&'static str> {
    Some(match name {
        ...
        // Ubm display server (Samsung)
        "ubm_device" => "pub enum ubm_device {}",
        "ubm_surface" => "pub enum ubm_surface {}",
        // OpenHarmony
...
/// One layout fact set for the `--layout-probe` binary: a type whose size and
/// alignment the probe prints, plus `(C member name, Rust field name)` pairs
/// for field-offset facts.  Collected at the emission site, so the probe can
/// never drift from what was actually emitted — and if it somehow did, the
/// probe binary would fail to compile against the generated crate.
pub(super) struct ProbeEntry {
    pub name: String,
    pub fields: Vec<(String, String)>,
}

/// Derive the probe entry for an emitted type.  Platform-protected types are
/// skipped (the C side would need `VK_USE_PLATFORM_*` defines and platform
/// headers); bitfield members are skipped (C `offsetof` on a bitfield is
/// ill-formed — the struct's size/alignment still pins the carrier packing);
/// union member offsets are trivially zero and carry no information.
fn probe_entry(t: &TypeDef) -> Option<ProbeEntry> {
    if !t.protect.0.is_empty() {
        return None;
    }
    let fields = match &t.payload {
        TypePayload::Members(members) if t.category != TypeCategory::Union => members
            .iter()
            .filter(|m| m.ty.bitfield.is_none())
            .map(|m| (m.name.clone(), super::abi::sanitize_ident(&m.name)))
            .collect(),
        _ => Vec::new(),
    };
    Some(ProbeEntry {
...
/// Classify every `StdVideo*` name the feature set references, by usage
/// shape.  A name used by value anywhere (plain member, or array element) is
/// one of the video std headers' enums — ABI `int`, so `i32` here; a name
/// only ever behind pointers is one of its records, emitted opaque (the
/// definitions live in the Vulkan video std headers outside vk.xml, and
/// Vulkan's own API surface passes the records by pointer only).  The layout
/// oracle verifies the enum assumption mechanically: the probe asserts
/// `i32`'s size/alignment against the C side's real enums from the bundled
/// vk_video headers, and every by-value use inside a probed Vulkan struct
/// pins the member offsets that follow it.
fn collect_std_video(fs: &FeatureSet) -> indexmap::IndexMap<String, bool> {
    let mut map: indexmap::IndexMap<String, bool> = indexmap::IndexMap::new();
    let mut record = |ty: &TypeRef| {
        if ty.base.starts_with("StdVideo") {
            let by_value = ty.pointers.is_empty();
            let slot = map.entry(ty.base.clone()).or_insert(false);
            *slot |= by_value;
        }
    };
    for t in &fs.types {
    ...
    for cmd in &fs.commands {
        ...
        record(&cmd.return_ty);
    }
    map
}

pub(super) fn emit_vk_types(
    fs: &FeatureSet,
    s: &mut String,
    probe: &mut Vec<ProbeEntry>,
) -> Result<()> {
    let std_video = collect_std_video(fs);

    // Names defined somewhere in this module: types, enum groups, the
    // video-std declarations, and the platform table.  Used to validate
    ...
    for t in &fs.types {
        ...
        if let Some(entry) = probe_entry(t) {
            probe.push(entry);
        }
    ...
    if !std_video.is_empty() {
        s.push_str(
            "\n// ── Video std types ─────────────────────────────────────────\n\
             // StdVideo* definitions live in the Vulkan video std headers,\n\
             // outside vk.xml.  The scalar (enum) types are ABI `int`; the\n\
             // records are opaque here — Vulkan's API surface passes them by\n\
             // pointer only, and their definitions belong to video-std\n\
             // bindings (cast raw pointers at the boundary).\n",
        );
        for (name, by_value) in &std_video {
            if *by_value {
                let _ = writeln!(s, "pub type {name} = i32;");
                // The layout oracle checks this against the C headers' enum.
                probe.push(ProbeEntry {
                    name: name.clone(),
                    fields: Vec::new(),
                });
...
fn emit_type(
    ...
    // hand-written const fns in the loader prelude.
...
fn check_base(
    ...
    if ty.base.starts_with("StdVideo") {
        // Module-level emission declares every referenced StdVideo* name
        // from the feature set's usage shapes (see `collect_std_video`), so
        // this only fires for a caller that skipped the collection — the
        // per-type corpus test's carve-out.
        bail!(
            "base type '{}' is a vk_video type; module emission declares \
             these from the feature set's StdVideo usage shapes",
            ty.base
        );
    }
    bail!(
        ...
        match f {
            Field::Plain(m) => {
                let _ = writeln!(
                    s,
                    "    pub {}: {},",
                    super::abi::sanitize_ident(&m.name),
                    vk_type(&m.ty)
                );
            }
            Field::Packed(run) => {
