...
| [rust/gl-triangle](rust/gl-triangle/) | Rust | The generated loader as a crate (`gloam_gl`), consumed via the Rust-native `winit` + `glutin` stack: a desktop GL 3.3 context, `Gl::load_gl(...)`, and a spinning triangle. |
...
The Rust example is a standalone Cargo workspace, so its `winit`/`glutin`
dependencies stay out of the main gloam build.
