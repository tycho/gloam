...
| `generator/rust/mod.rs` | Rust generator entry: emits a self-contained `#![no_std]` crate (`Cargo.toml`, `lib.rs`, one module per resolved spec) |
| `generator/rust/abi.rs` | C → Rust ABI translation: audited scalar map, `TypeRef` printers, derived base-type emission |
| `generator/rust/common.rs` | Spec-agnostic emission: packed name/offset tables, PFN ranges, XXH3 extension-hash tables, dispatch methods |
| `generator/rust/gl_loader.rs` | GL/GLES loader emission (`Gl::load_gl(...)`, version parse, extension detection) |
| `generator/rust/egl_loader.rs` | EGL loader emission (display + client extension phases) |
| `generator/rust/wgl_loader.rs` | WGL loader emission (`GetExtensionsStringARB` bootstrap) |
| `generator/rust/glx_loader.rs` | GLX loader emission (`glXQueryVersion` / server-string bootstrap) |
| `generator/rust/vk_loader.rs` | Vulkan loader emission (phased `initialize → load_instance → load_device` contract, `discover`) |
| `generator/rust/vk_types.rs` | Vulkan type emission (enum groups, handles, structs/unions, StdVideo, `--layout-probe`) |
...
| `layout_oracle.rs` | Rust ↔ C type-layout cross-check: the generated crate's `--layout-probe` facts compiled as C `_Static_assert`s |
